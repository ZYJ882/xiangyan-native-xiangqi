...
@Composable private fun XiangqiRules() = Column {
    ...
    RuleSection("十一、和棋与重复局面", listOf("双方可以在规则允许的情况下协议和棋；在正式比赛中，重复局面、长将、长捉和其他循环行为通常由裁判按竞赛规程处理。", "本应用会阻止自将、将帅照面，并对三次重复局面判和；若重复局面均为同一方被将，则按连续将军判负。复杂长捉争议仍应以具体赛事规程为准。"))
...
@Composable private fun AppRules() = Column {
    SettingIntro("本软件的对局规则", "本应用是离线人机对弈工具。所有开始、暂停和停止均由用户显式触发，应用不会在启动后自行运行 AI。")
